...
def solve_minor(matrix, i, j): # Найти минор элемента матрицы
    n = len(matrix)
    return [[matrix[row][col] for col in range(n) if col != j] for row in range(n) if row != i]

def solve_det(matrix): # Найти определитель матрицы
    n = len(matrix)
    if n == 1:
        return matrix[0][0]
    det = 0
...
def calc_s(dots, f): # Найти меру отклонения
    n = len(dots)
...
def calc_stdev(dots, f): # Найти среднеквадратичное отклонение
    n = len(dots)

    return sqrt(calc_s(dots, f) / n)


def lin_func(dots): # Линейная аппроксимация
    output_data = {}
    ...
    output_data['f'] = f
    output_data['str_f'] = "f = a*x + b"
    output_data['s'] = calc_s(dots, f)
    output_data['stdev'] = calc_stdev(dots, f)
    output_data['r'] = sum([(x[i] - sx/n)*(y[i]-sy/n) for i in range(n)])

    return output_data


def sqrt_func(dots): # Квадратичная аппроксимация
    output_data = {}
    ...
    output_data['f'] = f
    output_data['str_f'] = "f = a*x^2 + b*x + c"
    output_data['s'] = calc_s(dots, f)
    output_data['stdev'] = calc_stdev(dots, f)
...
def exp_func(dots): # Экспоненциальная аппроксимация
    output_data = {}
...
def log_func(dots): # Логарифмическая аппроксимация
    output_data = {}
...
def pow_func(dots): # Степенная аппроксимация
    output_data = {}
...
def plot(x, y, plot_x, plot_ys, labels): # Отрисовать графики полученных функций
...
def getdata_file(): # Получить данные из файла
    data = {'dots': []}
...
def main():
    print("\tЛабораторная работа №4 (5)")
    print("\tАппроксимация функций")

    print("\nВзять исходные данные из файла (y/n)?")
    inchoice = input("Режим ввода: ").lower()
    while (inchoice != 'y') and (inchoice != 'n'):
        print("Введите 'y' или 'n' для выбора способа ввода.")
    ...
    try:
        print("\n Коэффициент корреляции:", f"  r = {round(best_answer['r'], 4)}")
    except KeyError:
        None
